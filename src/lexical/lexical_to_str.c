...
static void reserve(char_vector_t *char_vector, size_t new_capacity) {
  if (new_capacity <= char_vector->capacity)
    return;
...
static void push_char(char_vector_t *char_vector, char c) {
  if (char_vector->size + 1 > char_vector->capacity)
    reserve(char_vector,
            char_vector->capacity == 0 ? 2 : char_vector->capacity * 2);
...
// this is used to convert a statically allocated const char* into a dynamic
// one, that can be freed must be freed
...
char *lexical_tokens_to_str(const lexical_tokens_vector_t *tokens) {
  if (!tokens)
    return str_copy("null tokens vector  !");
  if (tokens->size == 0)
    return str_copy("empty tokens vector");
