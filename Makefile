...
grapheur: $(GRAPHEUR_OBJS) $(COMON_OBJS)
	@mkdir -p $(BUILD_DIR)
	$(CC) $(GRAPHEUR_OBJS) $(COMON_OBJS) -o $(BUILD_DIR)/$(PROJECT_NAME) $(LDFLAGS)

lexical: $(LEXICAL_OBJS) $(COMON_OBJS)
	@mkdir -p $(BUILD_DIR)
	$(CC) $(LEXICAL_OBJS) $(COMON_OBJS) -o $(BUILD_DIR)/$(PROJECT_NAME) $(LDFLAGS)

dialogueur: $(DIALOGUEUR_OBJS) $(COMON_OBJS)
	@mkdir -p $(BUILD_DIR)
	$(CC) $(DIALOGUEUR_OBJS) $(COMON_OBJS) -o $(BUILD_DIR)/$(PROJECT_NAME) $(LDFLAGS)

evaluateur: $(EVALUATEUR_OBJS) $(COMON_OBJS)
	@mkdir -p $(BUILD_DIR)
	$(CC) $(EVALUATEUR_OBJS) $(COMON_OBJS) -o $(BUILD_DIR)/$(PROJECT_NAME) $(LDFLAGS)

syntaxique: $(SYNTAXIQUE_OBJS) $(COMON_OBJS)
	@mkdir -p $(BUILD_DIR)
	$(CC) $(SYNTAXIQUE_OBJS) $(COMON_OBJS) -o $(BUILD_DIR)/$(PROJECT_NAME) $(LDFLAGS)
